import * as React from 'react';
import OutlinedInput from '@mui/material/OutlinedInput';
import InputLabel from '@mui/material/InputLabel';
import MenuItem from '@mui/material/MenuItem';
import FormControl from '@mui/material/FormControl';
import ListItemText from '@mui/material/ListItemText';
import Select, { SelectChangeEvent } from '@mui/material/Select';
import Checkbox from '@mui/material/Checkbox';
import { useAtom } from 'jotai';
import productsAtom from '../atoms/Products';
import productsData from '../data/products.json'; // Sample transaction data

const ITEM_HEIGHT = 48;
const ITEM_PADDING_TOP = 8;
const MenuProps = {
  PaperProps: {
    style: {
      maxHeight: ITEM_HEIGHT * 4.5 + ITEM_PADDING_TOP,
      width: 250,
    },
  },
};

export default function MultipleSelectCheckmarks() {
  const [productCategory, setproductCategory] = React.useState<string[]>([]);
  const [products, setProducts] = useAtom(productsAtom);
  
  // Get all categories from products
  const allCategories = productsData.flatMap(productsData => productsData.categories);

  // Get unique categories
  const uniqueCategories = Array.from(new Set(allCategories));

  const handleChange = (event: SelectChangeEvent<typeof productCategory>) => {
    const {
      target: { value },
    } = event;

    if(value.length) {
      // Convert category to an array if it's a string
      const categoriesToFilter = typeof value === 'string' ? [value] : value;
      
      // Filter products by category
      const filteredProducts = productsData.filter(product =>
        categoriesToFilter.some(cat => product.categories.includes(cat))
      );
      setProducts(filteredProducts);
    } else {
      setProducts(productsData);
    }

    setproductCategory(
      // On autofill we get a stringified value.
      typeof value === 'string' ? value.split(',') : value,
    );
  };

  return (
    <div>
      <FormControl sx={{ m: 1, width: 300 }}>
        <InputLabel id="demo-multiple-checkbox-label">Filter Categories</InputLabel>
        <Select
          labelId="demo-multiple-checkbox-label"
          id="demo-multiple-checkbox"
          multiple
          value={productCategory}
          onChange={handleChange}
          input={<OutlinedInput label="Filter Categories" />}
          renderValue={(selected) => selected.join(', ')}
          MenuProps={MenuProps}
        >
          {uniqueCategories.map((name) => (
            <MenuItem key={name} value={name}>
              <Checkbox checked={productCategory.indexOf(name) > -1} />
              <ListItemText primary={name} />
            </MenuItem>
          ))}
        </Select>
      </FormControl>
    </div>
  );
}